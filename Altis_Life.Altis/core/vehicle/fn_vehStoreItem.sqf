...
_FuelTrucks = ["C_Van_01_fuel_F","B_Truck_01_fuel_F","O_Truck_02_fuel_F","O_Truck_03_fuel_F","I_Heli_Transport_02_F","O_Truck_03_device_F"]; //Fuel Trucks, who are allowed to carry fuel
...
case (!(_ctrl in _FuelStuff) && !(_Truck in _FuelTrucks)): {life_TankLaster = 2}; //NonFuel in NonFuelTruck -> Allowed
	case (!(_ctrl in _FuelStuff) && (_Truck in _FuelTrucks)): {
		if ((_Truck == "O_Truck_03_device_F") || (_Truck isKindOf "House_F") || (_Truck == "I_Heli_Transport_02_F") ) then {
			life_TankLaster = 2;
		} else {
			life_TankLaster = 3;
		};
