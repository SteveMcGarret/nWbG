...
*/
//if (!isServer || isDedicated) exitWith {}; Disable for Testing
...
if (isPlayer _unit) then {
	...
	[_unit] spawn {
		...
		//if (count _heliNear == 0) then {playSound "C130_exit"}; //Disable Sound atm
...
if (_saveLoadOut && !isNull (unitBackpack _unit) && (backpack _unit) != "b_parachute") then {
	...
	[_unit,_class,_magazines,_weapons,_items,_helmet,_altitude] spawn {
		...
		if (_altitude > 3040 && _helmet != "" && _helmet != "H_CrewHelmetHeli_I") then {(unitBackpack _unit) addItemCargoGlobal [_helmet, 1]}; //(not complete) do a check to see if there is available space
...
if (_altitude > 3040 && (headgear _unit) != "H_CrewHelmetHeli_I") then {_unit addHeadgear "H_CrewHelmetHeli_I"};
