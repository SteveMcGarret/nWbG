...
_unit spawn
{
	...
	_Timer ctrlSetText "Wenn du einen Medic rufst erhöht sich die Respawn Zeit aber deine Chancen werden sehr hoch wiederbelebt zu werden";
	sleep 10;

	life_maxTime = time + (life_respawn_timer * 60);
	_RespawnBtn ctrlEnable false;
	waitUntil {_Timer ctrlSetText format[localize "STR_Medic_Respawn",[(life_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString];
	round(life_maxTime - time) <= 0 OR isNull _this};
	_RespawnBtn ctrlEnable true;
	_Timer ctrlSetText localize "STR_Medic_Respawn_2";
};

[] spawn life_fnc_deathScreen;
[] spawn life_fnc_killedTLCredits;
